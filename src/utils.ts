...
import { AmizoneCredentials } from "./states/states";

export const newAmizoneClient = ({ username, password }: AmizoneCredentials) => {
  if (!username || !password) {
    throw new Error("credentials must be populated when creating new client!");
  }
  return new amizone.AmizoneServiceApi(new amizone.Configuration({
    username,
    password,
  }));
}
